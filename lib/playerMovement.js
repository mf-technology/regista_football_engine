...
function decideMovement(closestPlayer, team, opp, matchDetails) {
  const allActions = [
    'shoot', 'throughBall', 'pass', 'cross', 'tackle', 'intercept', 'slide',
    'run', 'sprint', 'cleared', 'boot', 'penalty'
  ]
  let {
...
function completeSlide(matchDetails, thisPlayer, team, opp) {
  ...
  if (common.isBetween(intensity, CARD_THRESHOLDS.YELLOW, CARD_THRESHOLDS.YELLOW_MAX)) {
    thisPlayer.stats.cards.yellow++
    if (thisPlayer.stats.cards.yellow == 2) {
      thisPlayer.stats.cards.red++
      setPlayerAsDisqualified(thisPlayer)
    }
  } else if (common.isBetween(intensity, CARD_THRESHOLDS.RED, CARD_THRESHOLDS.RED_MAX)) {
    thisPlayer.stats.cards.red++
    setPlayerAsDisqualified(thisPlayer)
  }
  if (opp.name == matchDetails.kickOffTeam.name) return setPositions.setSetpieceKickOffTeam(matchDetails)
...
function getMovement(player, action, opposition, ballX, ballY, matchDetails) {
  // Sık kullanılan değerleri önbelleğe al
  ...
  if (action === `wait` || ballActions.includes(action)) return [0, 0]
    ...
    return getInterceptMovement(player, opposition, position, pitchSize)
  } else if (action === `run`) {
...
function setPlayerAsDisqualified(thisPlayer) {
