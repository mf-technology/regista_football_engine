  ...
  for (let i = 0; i < players.length; i++) {
    ...
    if (thisPlayer.currentPOS[0] != 'NP') {
      ...
      if (thisPlayer.hasBall) {
        if (thisPlayer.skill.shooting > 80) {
          action = 'shoot'
        }
      }
      ...
      let xPosition = common.isBetween(thisPlayer.currentPOS[0], ballPosition[0] - 3, ballPosition[0] + 3)
      let yPosition = common.isBetween(thisPlayer.currentPOS[1], ballPosition[1] - 3, ballPosition[1] + 3)
      let samePositionAsBall = thisPlayer.currentPOS[0] === ballPosition[0] && thisPlayer.currentPOS[1] === ballPosition[1]
      let closeWithPlayer = !!((xPosition && yPosition && matchDetails.ball.withPlayer == false))
      if (xPosition && yPosition && matchDetails.ball.withTeam !== team.teamID) {
        if (samePositionAsBall) {
          if (matchDetails.ball.withPlayer === true && thisPlayer.hasBall === false && matchDetails.ball.withTeam !== team.teamID) {
        ...
        } else if (matchDetails.ball.withPlayer === true && thisPlayer.hasBall === false && matchDetails.ball.withTeam !== team.teamID) {
          if (action === `slide`) matchDetails = completeSlide(matchDetails, thisPlayer, team, opp)
        } else {
...
function checkOffside(matchDetails, player) {
  if (isOffside(player)) {
    ...
    // Ofsayt durumunda yapılacak işlemler
  }
}

function isOffside(player) {
  // Ofsayt kontrolü
