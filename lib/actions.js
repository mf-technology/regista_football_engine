...
function setFoul(matchDetails, team, player, thatPlayer) {
  ...
  if (player.stats.tackles.fouls >= 2) {
    player.stats.cards.yellow++;
  }
  if (player.stats.cards.yellow >= 2) {
    player.stats.cards.red++;
    player.currentPOS = ['NP', 'NP']; // Oyuncu oyundan çıkar
  }
}
...
function checkFreeKick(matchDetails, player, thatPlayer) {
  if (isFoulOutsidePenaltyArea(player, thatPlayer)) {
    matchDetails.iterationLog.push(`Free kick awarded to: ${player.name}`);
    setFreeKick(matchDetails, player);
  }
}

function isFoulOutsidePenaltyArea(player, thatPlayer) {
  // Ceza sahası dışındaki faul kontrolü
  return (player.currentPOS[0] < penaltyAreaX1 || player.currentPOS[0] > penaltyAreaX2);
}

function setFreeKick(matchDetails, player) {
  // Serbest vuruş için topu uygun bir yere yerleştir
  matchDetails.ball.position = [freeKickX, freeKickY];
  matchDetails.ball.withPlayer = true;
  matchDetails.ball.Player = player.playerID;
  matchDetails.ball.withTeam = player.teamID;
}

function checkCornerKick(matchDetails) {
  if (isBallOutOfPlay(matchDetails.ball.position)) {
    matchDetails.iterationLog.push(`Corner kick awarded`);
    setCornerKick(matchDetails);
  }
}

function isBallOutOfPlay(ballPosition) {
  // Topun dışarıda olup olmadığını kontrol et
  return (ballPosition[0] < 0 || ballPosition[0] > pitchWidth);
}

function setCornerKick(matchDetails) {
  // Köşe vuruşu için topu köşe noktasına yerleştir
  matchDetails.ball.position = [cornerX, cornerY];
  matchDetails.ball.withPlayer = true;
  matchDetails.ball.Player = null; // Köşe vuruşunu kullanacak oyuncu
  matchDetails.ball.withTeam = null; // Köşe vuruşunu kullanacak takım
}

function checkPenalty(matchDetails, player, thatPlayer) {
  if (isFoulInPenaltyArea(player, thatPlayer)) {
    matchDetails.iterationLog.push(`Penalty awarded to: ${player.name}`);
    // Penaltı atışı için gerekli işlemleri yap
    setPenaltyKick(matchDetails, player);
  }
}

function isFoulInPenaltyArea(player, thatPlayer) {
  // Ceza sahası kontrolü
  return (player.currentPOS[0] >= penaltyAreaX1 && player.currentPOS[0] <= penaltyAreaX2 && 
          player.currentPOS[1] >= penaltyAreaY1 && player.currentPOS[1] <= penaltyAreaY2);
}

function setPenaltyKick(matchDetails, player) {
  // Penaltı atışı için topu oyuncunun önüne yerleştir
  matchDetails.ball.position = [penaltySpotX, penaltySpotY];
  matchDetails.ball.withPlayer = true;
  matchDetails.ball.Player = player.playerID;
  matchDetails.ball.withTeam = player.teamID;
}
...
function updateStatistics(matchDetails) {
  // Sadece değişen istatistikleri güncelleyin
  if (matchDetails.ball.withPlayer) {
    // Gol atıldığında veya faul yapıldığında istatistikleri güncelleyin
    // Örnek: matchDetails.kickOffTeamStatistics.goals++;
  }
}
