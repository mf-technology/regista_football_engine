...
function checkGoalScored(matchDetails) {
  ...
  const goalLineY = 0;
  if (ball.position[1] <= goalLineY) {
    matchDetails.iterationLog.push(`Goal Scored by: ${ball.lastTouch.playerName}`);
    // İstatistikleri yalnızca gol atıldığında güncelle
    if (ball.lastTouch.teamID === matchDetails.kickOffTeam.teamID) {
